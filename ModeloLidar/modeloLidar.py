...
def Impresion_puntos(df, lidar_fixed_ang, width, length, num):
    ...
    fig,ax = plt.subplots()
    if num == 0:
        ax.set_title("Un escaneo",fontsize=14,fontweight="bold")
    else:
        ax.set_title("Escaneo completo del campo",fontsize=14,fontweight="bold")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
...
def Impresion_recorrido(df_p, df_r, width, length):
    # Imprimir en pantalla
    fig,ax = plt.subplots()
    ax.set_title("Recorrido",fontsize=14,fontweight="bold")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_xlim(0, width)
    ax.set_ylim(0, length)
...
def Proyeccion_angular_lidar(alpha, tX, tY, tZ, diccionario ):
    # Variables--------------------------------------
    
    lidar_fixed_ang = diccionario['angulos_lidar']
    lidar_fixed_ang = np.array(list(map(float,lidar_fixed_ang)))
    angular_ring_resolution = diccionario['Resolucion angular de anillos'][0]
    # lidar_fixed_ang = np.array([-17,-14.17,-11.33,-8.50,-5.67,-2.83,0,3])
...
def recorrido(width, length, starting_point, step_limit_x, step_limit_y, dis_furrows, waypoint_distance):
    ...
    if(waypoint_distance < 1):
        waypoint_distance = round(waypoint_distance, 2)
        DISTANCIA = 0.01
    else:
        waypoint_distance = round(waypoint_distance)
        DISTANCIA = 1
    while(furrow):
        if(num_furrow == 0):
...
def transformada_del_plano(scannig_route_matrix, pcd_scan, target_ring, target_p):
    ...
    for route_x, route_y, route_z in scannig_route_matrix:
...
def points_filtered(points_matrix, width, length, lidar_fixed_ang, alpha, resultado):
    Xmax = width
    Xmin = -1
    Ymax = length
    Ymin = -1
    ...
    points_m2 = round(len(points_matrix[:,0]) / (length * width),4)               # numero de puntos por m^2
    # print(f"numero de puntos totales por m^2: {points_m2}")
    average_distance = np.mean( points_matrix,axis = 0)
    average_distance = round(average_distance[4],4)
    # print(f"la distancia promedio del lidar a los puntos es: {average_distance[4]}")
    average_ang = round((np.sum(lidar_fixed_ang + alpha)) / len(lidar_fixed_ang ),4)
...
def simulacion(diccionario,dato):
    alpha = diccionario['Angulo del lidar'][0]                                    # Ángulo del lidar
    ...
    starting_point = [1,1]                                          # Es el punto de partifa en los ejes (x,y)
    step_limit_x = [1,width-1]                                    # Es el limite minimo y maximo del eje X en el recorrido [min, max]
    step_limit_y = [1,length-1]                                   # Es el limite minimo y maximo del eje y en el recorrido [min, max]
    ...
    #funciones
    pcd_scan, target_ring, target_p, lidar_fixed_ang = Proyeccion_angular_lidar(alpha, tX, tY, tZ, diccionario)
    scannig_route_matrix, resultado = recorrido(width, length, starting_point, step_limit_x, step_limit_y, dis_furrows, waypoint_distance)
    ...
    resultados = points_filtered(points_matrix, width, length, lidar_fixed_ang, alpha, resultado)
