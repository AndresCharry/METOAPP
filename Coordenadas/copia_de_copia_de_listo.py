...
def main(num_zonas_en_campo, puntos_por_area, pph, latitud, longitud):
    ...
    if lidar == 1:
      ...
      marcadores_en_area_x,marcadores_en_area_y = creando_puntos(0,1,x_R,y_R,pph,vertices_x,vertices_y)
      vector_marcadores_dentrox,vector_marcadores_dentroy= calculo_puntos_dentro(1,2
                        ,marcadores_en_area_x,marcadores_en_area_y,1)

      p_d_lat_mk,p_d_lon_mk = selec_point(vector_marcadores_dentrox[0], vector_marcadores_dentroy[0], 5)
      puntos_markers_latitud = p_d_lat_mk; puntos_markers_lontud = p_d_lon_mk
      plt.figure(figsize=(10,10))
      ax = plt.axes()
      for i in range (len(vector_puntos_dentrox)):
        plt.plot(vertices_x[i+1], vertices_y[i+1], alpha=1, linewidth=2, solid_capstyle='round', zorder=2,label = "Area1")
      ...
      for i in range(len(puntos_markers_latitud)):
        markers(puntos_markers_latitud[i]-tam, puntos_markers_lontud[i]-tam, tam,ax)
      # recreando areas
